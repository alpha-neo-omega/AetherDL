...
| M5g framed player detected | PASS (automated) | NOT EXECUTED | Chromium, real browser: a watch page holding no media of its own, wrapping the disguised player in a same-origin iframe. The top frame is asserted empty (`videos: 0, iframes: 1`) and the frame reports the disguised playlist in its own right (`tests/e2e/chromium.spec.ts`). Cross-origin frames need a host grant the browser only gives on a real toolbar click, so those are not automated |
