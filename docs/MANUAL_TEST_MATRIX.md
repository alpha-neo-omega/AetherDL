...
| M5d | Refuse a split-track stream | Download an HLS master whose audio is a separate rendition, or a DASH manifest with separate audio/video AdaptationSets | Refused with a stated reason before any segment is fetched. A silent video is never saved |
...
| M5d split-track refused | PASS (automated) | NOT EXECUTED | Chromium, real browser: an HLS master with a separate audio rendition failed with `stream-hls-separate-audio` and message key `error.download.stream.tracks`; the native download count was unchanged 0 → 0 (`tests/e2e/stream-chromium.spec.ts`). DASH split-track refusal is unit-tested |
