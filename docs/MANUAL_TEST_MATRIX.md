...
| M5g | Media inside an iframe | Open a page whose player is an embedded iframe (most embedded players are), open AetherDL | The media inside the frame is listed, not just whatever the top document happens to hold |
...
| M17b | Embedded player | Open a page whose player is embedded from another origin | The popup names that site and offers the grant instead of "No media detected"; allowing it detects the media; the grant is listed in Settings and revocable |
...
| M5g framed player detected | PASS (automated) | NOT EXECUTED | Chromium, real browser: a watch page holding no media of its own, wrapping the disguised player in a same-origin iframe. The top frame is asserted empty (`videos: 0, iframes: 1`) and the frame reports the disguised playlist in its own right (`tests/e2e/chromium.spec.ts`). Cross-origin frames need a host grant the browser only gives on a real prompt — see M17b, which covers everything up to that prompt |
| M17b embedded player | PARTIAL (automated) | NOT EXECUTED | Chromium, real browser: a watch page embedding a genuinely cross-origin frame. The top document reported that origin and only that origin, `detection/embeds` answered with it over the real message bus, and `permissions.getAll()` confirmed **nothing was granted by asking** (`tests/e2e/chromium.spec.ts`). What automation cannot do is accept the browser's own permission prompt, so the grant → inject → detect half is covered by `tests/unit/ui/popup/app.test.tsx` (offer, grant ordering, decline, already-granted) and `tests/unit/runtime/background/runtime.test.ts` (frames re-entered on refresh). Confirmed red with the feature removed |
