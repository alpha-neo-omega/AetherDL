...
| A host that rate-limits | Segments are retried **where they failed**, with exponential backoff, bounded by an attempt count and a wall-clock budget per segment. Measured against a real anti-leech host: the same segment size went from 150 ms to 44 s as a download progressed. Retrying at the segment matters because the queue's retry restarts assembly from the first segment. A host that stops answering is reported as rate-limiting, naming how far the download got, rather than as a broken stream. AetherDL **MUST NOT** respond by spoofing a `Referer`, forging headers, or otherwise altering requests to get past an access control ([§3](#3-non-goals), [§12.6](#126-network-observation-and-detection-probing)) |
